...
enum Command {
    ...
    AddAccount { config: PathBuf },
    Accounts,
    Mailboxes { account: String },
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    match cli.command {
        Command::AddAccount { config } => {
            ...
            if account.id.is_empty() || account.imap_host.is_empty() || account.username.is_empty() || account.imap_port == 0 {
        ...
        Command::Mailboxes { account } => println!("{}", serde_json::to_string_pretty(&db.list_mailboxes(&account)?)?),
        Command::Messages { mailbox, offset, limit } => println!("{}", serde_json::to_string_pretty(&db.list_messages(&mailbox, offset, limit)?)?),
        Command::Sync { account } => {
            let account = db.list_accounts()?.into_iter().find(|a| a.id == account).ok_or("Account not found")?;
