...
struct SenderIdentity: Equatable {
    ...
    private static func parse(_ value: String) -> (name: String, address: String?) {
        if let opening = value.lastIndex(of: "<"), let closing = value[opening...].firstIndex(of: ">") {
            let candidate = String(value[value.index(after: opening) ..< closing])
            if let address = normalizedEmail(candidate) {
...
struct ExternalLinkDestination: Identifiable, Equatable {
    ...
    var host: String {
        url.host ?? "Unknown destination"
    }

    var warning: String? {
        guard let scheme = url.scheme?.lowercased(), ["http", "https"].contains(scheme),
              let destinationHost = url.host?.lowercased()
        else {
        ...
        let visibleHost = Self.visibleHost(in: visibleText)
        if let visibleHost, !relatedDomains(visibleHost, destinationHost) {
