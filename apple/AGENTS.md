...
- Mailbox message paging replaces the displayed window; the current extra-row probe establishes whether another page exists, and an exactly full page alone does not. Conversation timelines prepend/append bounded pages without treating them as mailbox-window replacement. Reset paging and selection when changing mailboxes.
...
- Mailbox flags and offline actions are per-membership state. Keep optimistic mark/star/move projections identifiable until replay, and use the store's durable pending/failed operation state rather than inventing local persistence or promising automatic retry.
